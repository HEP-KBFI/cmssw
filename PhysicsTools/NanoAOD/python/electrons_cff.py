...
electronTable = cms.EDProducer("SimpleCandidateFlatTableProducer",
    ...
    variables = cms.PSet(CandVars,
        ...
        deltaPhiSC = Var("superCluster().phi()-phi()",float,doc="delta phi (SC,ele) with sign",precision=10),
        deltaEtaSC_trackatVtx = Var("deltaEtaSuperClusterTrackAtVtx()",float,doc="HLT safe delta eta (SC,ele) with sign",precision=10),
        deltaPhiSC_trackatVtx = Var("deltaPhiSuperClusterTrackAtVtx()",float,doc="HLT safe delta phi (SC,ele) with sign",precision=10),
        ...
        pfRelIso04_all = Var("userFloat('PFIsoAll04')/pt",float,doc="PF relative isolation dR=0.4, total (with rho*EA PU corrections)"),
        ...
        jetPtRatio_TTHMVA = Var("?userCand('jetForLepJetVar').isNonnull()?userFloat('ptRatio'):1.0/(1.0+userFloat('PFIsoAll04')/pt)",float,doc="jetPtRatio variable used by TTH MVA"),
        jetBTagCSV_TTHMVA = Var("?userCand('jetForLepJetVar').isNonnull()?userCand('jetForLepJetVar').bDiscriminator('pfCombinedInclusiveSecondaryVertexV2BJetTags'):-1.",float,doc="jetBTagCSV variable used by TTH MVA"),
        jetBTagDeepCSV_TTHMVA = Var("?userCand('jetForLepJetVar').isNonnull()?userCand('jetForLepJetVar').bDiscriminator('pfDeepCSVJetTags:probb')+userCand('jetForLepJetVar').bDiscriminator('pfDeepCSVJetTags:probbb'):-1.",float,doc="jetBTagDeepCSV variable"),
    ),
