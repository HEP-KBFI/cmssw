...
genWeightsTable = cms.EDProducer("GenWeightsTableProducer",
    ...
    namedWeightIDs = cms.vstring(*tuple(map(lambda x: 'rwgt_%d' % x, range(1, 70)))),
    namedWeightLabels = cms.vstring(*tuple(map(lambda x: 'rwgt_%d' % x, range(1, 70)))),
...
nanoSequence = cms.Sequence(
        ...
        linkedObjects  +
        jetTables + muonTables + tauTables + electronTables + photonTables + globalTables + vertexTables + metTables + simpleCleanerTable + triggerObjectTables + isoTrackTables + boostedTables +
	l1bits)
